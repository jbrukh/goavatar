...
// Convert bytes to uint16s. Using bit shifting
// for better efficiency over encoding/binary package,
// since we know we're only dealing with integers.
func toUint16(b []byte) uint16 {
	return uint16(b[0])<<8 | uint16(b[1])
}

// Convert bytes to uint32s. Using bit shifting
// for better efficiency over encoding/binary package,
// since we know we're only dealing with integers.
...
func toUint32From3Bytes(b []byte) uint32 {
	return uint32(b[0])<<16 | uint32(b[1])<<8 | uint32(b[2])
}

func parseSampleRate(b byte) uint16 {
	v := b >> 6
	if v == 0x00 {
		return 250
	} else if v == 0x01 {
	...
	panic(ParseErrf("Unknown sample rate: %x", v))
}

// ParseErr occurs when we have 
// trouble parsing the stream
...
func ParseErrf(msg string, args ...interface{}) ParseErr {
	return ParseErr(fmt.Sprintf(msg, args))
}
